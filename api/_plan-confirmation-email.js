/**
 * Shared plan / welcome email (Resend). Used by the HTTP route and by webhooks
 * so we never self-fetch over HTTP (Vercel Deployment Protection breaks that).
 */
...
/**
 * @param {{ userEmail: string, userName?: string|null, plan: string, checkoutSessionId?: string|null }} payload
 * @returns {Promise<{ ok: true, messageId: string } | { ok: false, status: number, error: string }>}
 */
export async function sendPlanConfirmationEmail(payload) {
  const { userEmail, userName, plan, checkoutSessionId } = payload || {};

  if (!process.env.RESEND_API_KEY) {
    return { ok: false, status: 500, error: "Email provider not configured" };
  }
  if (!userEmail || !plan) {
    return { ok: false, status: 400, error: "Missing email or plan" };
  }
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <div class="greeting">
                Hi ${userName ? String(userName).split(" ")[0] : "there"},
              </div>
            ...
            <div class="footer">
              ...
              <div class="receipt-ref">
                Order ID: ${checkoutSessionId ? String(checkoutSessionId).substring(0, 12) : "N/A"}
              </div>
            </div>
          </div>
        </body>
  ...
  try {
    ...
    return { ok: true, messageId: messageId || "sent" };
  } catch (error) {
